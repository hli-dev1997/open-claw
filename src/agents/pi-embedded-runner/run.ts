...
import { formatNodeLog, previewLogValue, type NodeLogFields } from "../../logging/node-log.js";
...
function buildTraceToolSummary(params: {
  ...
  return {
    ...
    tools,
    failures: params.failureCount ?? (params.hasUnresolvedFailure ? 1 : 0),
  };
}
...
// 核心 Agent 运行循环：组装历史记忆、构建 Prompt、调用 LLM、处理重试。
...
export async function runEmbeddedPiAgent(
  ...
  console.log(
    formatNodeLog({
      id: "agent.embedded.enter",
      name: "进入嵌入式Agent",
      summary: "进入 runEmbeddedPiAgent 主运行循环",
      fields: {
        runId: params.runId,
        sessionId: params.sessionId,
  ...
  const queueEnqueuedAt = Date.now();
  console.log(
    formatNodeLog({
      id: "queue.enqueue",
      name: "进入会话队列",
      summary: "按 sessionKey 串行化，避免同一会话并发写历史",
      fields: {
        runId: params.runId,
        lane: sessionLane,
      },
  ...
  return enqueueSession(async () => {
    ...
    console.log(
      formatNodeLog({
        id: "queue.start",
        name: "开始执行队列任务",
        summary: "当前 run 获得 session lane 执行权",
        fields: {
          runId: params.runId,
          lane: sessionLane,
          waitedMs: Date.now() - queueEnqueuedAt,
        },
      }),
    );
    const queueStartedAt = Date.now();
    let runSummaryLogged = false;
    const emitRunSummary = (fields: NodeLogFields) => {
      if (runSummaryLogged) {
        return;
      }
      ...
      console.log(
        formatNodeLog({
          id: "agent.run.summary",
          name: "运行摘要",
          summary: "本次 Agent run 结束",
          fields,
        }),
      );
    };
    const summarizeRunResult = (result: EmbeddedPiRunResult): NodeLogFields => {
      const finalText =
        result.meta.finalAssistantVisibleText ??
        result.payloads?.map((payload) => payload.text ?? "").join("") ??
        "";
      const winnerProvider =
        result.meta.executionTrace?.winnerProvider ??
        result.meta.agentMeta?.provider ??
        params.provider;
      const winnerModel =
        result.meta.executionTrace?.winnerModel ?? result.meta.agentMeta?.model ?? params.model;
      const hasErrorPayload = result.payloads?.some((payload) => payload.isError === true) ?? false;
      return {
        status: result.meta.aborted || result.meta.error || hasErrorPayload ? "error" : "ok",
        runId: params.runId,
        durationMs: result.meta.durationMs,
        model: `${winnerProvider}/${winnerModel}`,
        modelCalls: result.meta.modelCalls ?? result.meta.executionTrace?.attempts?.length ?? 0,
        toolCalls: result.meta.toolSummary?.calls ?? 0,
        tools: result.meta.toolSummary?.tools.join(","),
        toolErrors: result.meta.toolSummary?.failures ?? 0,
        finalChars: finalText.length,
      };
    };
    try {
      const runResult = await enqueueGlobal(async (): Promise<EmbeddedPiRunResult> => {
        throwIfAborted();
        ...
        try {
          ...
          while (true) {
            ...
            const attemptToolSummary = buildTraceToolSummary({
              toolMetas: attempt.toolMetas,
              failureCount: attempt.toolFailureCount,
              hasUnresolvedFailure: Boolean(attempt.lastToolError),
            });
            ...
            const replayInvalid = resolveReplayInvalidForAttempt(null);
            const livenessState: EmbeddedRunLivenessState = attempt.yieldDetected
              ? "paused"
              : resolveRunLivenessState({
                  payloadCount,
                  aborted,
            ...
            const finalTextForLog = finalAssistantVisibleText ?? "";
            const toolCallsForLog = attemptToolSummary?.calls ?? 0;
            emitRunSummary({
              status: aborted || timedOut ? "error" : "ok",
              runId: params.runId,
              durationMs: Date.now() - started,
              model: `${reportedModelRef.provider}/${reportedModelRef.model}`,
              modelCalls,
              toolCalls: toolCallsForLog,
              tools: attemptToolSummary?.tools.join(","),
              toolErrors: attemptToolSummary?.failures ?? (attempt.lastToolError ? 1 : 0),
              lastTool: attempt.lastToolError?.toolName,
              finalChars: finalTextForLog.length,
            });
            return {
      ...
      emitRunSummary(summarizeRunResult(runResult));
      return runResult;
    } catch (err) {
      emitRunSummary({
        status: "error",
        ...
        errorPreview: previewLogValue(formatErrorMessage(err), 120),
      });
      throw err;
    } finally {
      console.log(
        formatNodeLog({
          id: "queue.done",
          name: "队列任务完成",
          summary: "当前 run 释放 session lane",
          fields: {
            runId: params.runId,
            lane: sessionLane,
            durationMs: Date.now() - queueStartedAt,
