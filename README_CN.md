...
**OpenClaw** 是一个运行在你自有设备上的 _个人 AI 助手_。
它在你已经使用的各个渠道上为你提供回答。它可以在 macOS/iOS/Android 上实现语音交互，并能够渲染一个你可控制的实时 Canvas。Gateway 只是控制平面 — 产品本身才是助手。

如果你想要一个个人、单用户、感觉本地化、快速且始终在线的助手，这就是你需要的。

支持的渠道包括：WhatsApp、Telegram、Slack、Discord、Google Chat、Signal、iMessage、BlueBubbles、IRC、Microsoft Teams、Matrix、飞书、LINE、Mattermost、Nextcloud Talk、Nostr、Synology Chat、Tlon、Twitch、Zalo、Zalo 个人版、微信、QQ、WebChat。
...
推荐设置：在终端中运行 `openclaw onboard`。
OpenClaw Onboard 会逐步引导你完成 gateway、工作区、渠道和技能的设置。这是推荐的 CLI 设置路径，适用于 **macOS、Linux 和 Windows（通过 WSL2；强烈推荐）**。
支持 npm、pnpm 或 bun。
...
模型说明：虽然支持众多提供商和模型，但建议优先选择你信任且已在使用的提供商的主流旗舰模型。参见[入门向导](https://docs.openclaw.ai/start/onboarding)。

## 安装（推荐）

运行环境：**Node 24（推荐）或 Node 22.14+**。
...
# 发送消息
openclaw message send --target +1234567890 --message "来自 OpenClaw 的问候"

# 与助手对话（可选地将回复发送到任何已连接的渠道：WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/IRC/Microsoft Teams/Matrix/飞书/LINE/Mattermost/Nextcloud Talk/Nostr/Synology Chat/Tlon/Twitch/Zalo/Zalo 个人版/微信/QQ/WebChat）
openclaw agent --message "发货清单" --thinking high
```

正在升级？[更新指南](https://docs.openclaw.ai/install/updating)（并运行 `openclaw doctor`）。

模型配置 + CLI：[模型](https://docs.openclaw.ai/concepts/models)。认证配置文件轮换 + 回退：[模型故障切换](https://docs.openclaw.ai/concepts/model-failover)。

## 安全默认设置（DM 访问）

OpenClaw 连接到真实的通讯平台。请将收到的私信视为**不可信输入**。
...
- **DM 配对**（`dmPolicy="pairing"` / `channels.discord.dmPolicy="pairing"` / `channels.slack.dmPolicy="pairing"`；旧版：`channels.discord.dm.policy`、`channels.slack.dm.policy`）：未知发送者会收到一个短配对码，机器人不会处理他们的消息。
- 批准方式：`openclaw pairing approve <渠道> <配对码>`（之后发送者会被添加到本地允许列表中）。
- 公开入站 DM 需要显式选择：设置 `dmPolicy="open"` 并在渠道允许列表（`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`；旧版：`channels.discord.dm.allowFrom`、`channels.slack.dm.allowFrom`）中包含 `"*"`。

运行 `openclaw doctor` 以发现风险或配置不当的 DM 策略。

## 亮点

- **[本地优先的 Gateway](https://docs.openclaw.ai/gateway)** — 会话、渠道、工具和事件的单一控制平面。
- **[多渠道收件箱](https://docs.openclaw.ai/channels)** — WhatsApp、Telegram、Slack、Discord、Google Chat、Signal、BlueBubbles (iMessage)、iMessage（旧版）、IRC、Microsoft Teams、Matrix、飞书、LINE、Mattermost、Nextcloud Talk、Nostr、Synology Chat、Tlon、Twitch、Zalo、Zalo 个人版、微信、QQ、WebChat、macOS、iOS/Android。
- **[多智能体路由](https://docs.openclaw.ai/gateway/configuration)** — 将入站渠道/账号/对端路由到隔离的智能体（工作区 + 每个智能体独立会话）。
...
- **[实时 Canvas](https://docs.openclaw.ai/platforms/mac/canvas)** — 基于 [A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui) 的智能体驱动视觉工作空间。
...
- **[入门向导](https://docs.openclaw.ai/start/wizard) + [技能](https://docs.openclaw.ai/tools/skills)** — 向导驱动的设置，支持内置/托管/工作区技能。

## 安全模型（重要）

- 默认：工具在宿主机上为 `main` 会话运行，因此仅你使用时智能体拥有完全访问权限。
- 群组/频道安全：设置 `agents.defaults.sandbox.mode: "non-main"` 可以在沙箱中运行非 `main` 会话。Docker 是默认沙箱后端；同时也支持 SSH 和 OpenShell 后端。
- 典型的沙箱默认设置：允许 `bash`、`process`、`read`、`write`、`edit`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`；禁止 `browser`、`canvas`、`nodes`、`cron`、`discord`、`gateway`。
- 在对外暴露任何内容之前，请阅读[安全](https://docs.openclaw.ai/gateway/security)、[沙箱](https://docs.openclaw.ai/gateway/sandboxing)和[配置](https://docs.openclaw.ai/gateway/configuration)文档。

## 操作员快捷参考

- 聊天命令：`/status`、`/new`、`/reset`、`/compact`、`/think <级别>`、`/verbose on|off`、`/trace on|off`、`/usage off|tokens|full`、`/restart`、`/activation mention|always`
- 会话工具：`sessions_list`、`sessions_history`、`sessions_send`
- 技能注册中心：[ClawHub](https://clawhub.ai)
- 架构概览：[架构](https://docs.openclaw.ai/concepts/architecture)

## 按目标分类的文档

- 新手入门：[快速入门](https://docs.openclaw.ai/start/getting-started)、[入门向导](https://docs.openclaw.ai/start/wizard)、[更新](https://docs.openclaw.ai/install/updating)
...
- 配置 + 安全：[配置](https://docs.openclaw.ai/gateway/configuration)、[安全](https://docs.openclaw.ai/gateway/security)、[沙箱](https://docs.openclaw.ai/gateway/sandboxing)
- 远程 + 网页：[Gateway](https://docs.openclaw.ai/gateway)、[远程访问](https://docs.openclaw.ai/gateway/remote)、[Tailscale](https://docs.openclaw.ai/gateway/tailscale)、[Web 界面](https://docs.openclaw.ai/web)
- 工具 + 自动化：[工具](https://docs.openclaw.ai/tools)、[技能](https://docs.openclaw.ai/tools/skills)、[定时任务](https://docs.openclaw.ai/automation/cron-jobs)、[Webhook](https://docs.openclaw.ai/automation/webhook)、[Gmail Pub/Sub](https://docs.openclaw.ai/automation/gmail-pubsub)
- 内部原理：[架构](https://docs.openclaw.ai/concepts/architecture)、[智能体](https://docs.openclaw.ai/concepts/agent)、[会话模型](https://docs.openclaw.ai/concepts/session)、[Gateway 协议](https://docs.openclaw.ai/reference/rpc)
- 故障排除：[渠道故障排除](https://docs.openclaw.ai/channels/troubleshooting)、[日志](https://docs.openclaw.ai/logging)、[文档首页](https://docs.openclaw.ai)

## 应用（可选）

Gateway 本身已能提供出色的体验。所有应用都是可选的，用于增加额外功能。

如果你计划构建/运行伴侣应用，请按照下面的平台操作指南操作。

### macOS (OpenClaw.app)（可选）

- Gateway 的菜单栏控制和健康状态监控。
- 语音唤醒 + 按讲通话覆盖层。
- WebChat + 调试工具。
- 通过 SSH 远程控制 Gateway。

注意：macOS 权限需要在重建后持续生效，需要使用签名构建（参见 [macOS 权限](https://docs.openclaw.ai/platforms/mac/permissions)）。
...
操作指南：[iOS 连接](https://docs.openclaw.ai/platforms/ios)。

### Android 节点（可选）

- 通过设备配对连接为 WS 节点（`openclaw devices ...`）。
- 提供连接/聊天/语音标签页以及 Canvas、相机、屏幕录制和 Android 设备命令系列。
- 操作指南：[Android 连接](https://docs.openclaw.ai/platforms/android)。

## 从源码构建（开发）

推荐使用 `pnpm` 从源码构建。Bun 可选用于直接运行 TypeScript。
...
如果需要从当前代码检出构建 `dist/` 目录（用于 Node、打包或发布验证），请运行：
...
`pnpm openclaw setup` 会写入 `pnpm gateway:watch` 所需的本地配置/工作区。重复运行是安全的，但通常只在首次设置或重置本地状态时需要。`pnpm gateway:watch` 不会重建 `dist/control-ui`，因此在 `ui/` 更改后需要重新运行 `pnpm ui:build`；或者在迭代 Control UI 时使用 `pnpm ui:dev`。如果你想让此代码检出直接运行入门向导，使用 `pnpm openclaw onboard --install-daemon`。

注意：`pnpm openclaw ...` 直接运行 TypeScript（通过 `tsx`）。`pnpm build` 生成 `dist/` 用于通过 Node / 打包的 `openclaw` 二进制文件运行，而 `pnpm gateway:watch` 在开发循环中按需重建运行时。

## 开发渠道

- **stable（稳定版）**：标记发布版本（`vYYYY.M.D` 或 `vYYYY.M.D-<补丁号>`），npm dist-tag `latest`。
- **beta（测试版）**：预发布标签（`vYYYY.M.D-beta.N`），npm dist-tag `beta`（macOS 应用可能缺失）。
- **dev（开发版）**：`main` 分支的最新提交，npm dist-tag `dev`（发布时）。
...
- 技能：`~/.openclaw/workspace/skills/<技能名>/SKILL.md`。
...
[![Star 历史图表](https://api.star-history.com/svg?repos=openclaw/openclaw&type=date&legend=top-left)](https://www.star-history.com/#openclaw/openclaw&type=date&legend=top-left)
...
参见 [CONTRIBUTING.md](CONTRIBUTING.md) 了解贡献指南、维护者信息以及如何提交 PR。
欢迎 AI/氛围驱动编码的 PR！🤖

特别感谢 [Mario Zechner](https://mariozechner.at/) 的支持以及
[pi-mono](https://github.com/badlogic/pi-mono) 项目。
